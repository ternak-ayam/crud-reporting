...
if ($action == 'raw-material-arrival') {
    ...
    $rawMaterial->insertRawMaterialLog($_POST['id'], $_POST['qty'], $_SESSION['name'],
    $_POST['origin'], $_SESSION['ws'], 'arrival');

    header("location:raw-material-arrival.php?message=success");
} elseif ($action == 'wip-arrival') {

    $wip->sendWIP($_POST['id'], $_POST['qty']);
    $wip->insertWIPLog($_POST['id'], $_POST['qty,'], $_SESSION['name'], $_POST['origin'],
    $_SESSION['ws'], 'arrival');
